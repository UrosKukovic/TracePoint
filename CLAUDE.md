...
Engineering discipline for AI-assisted work on TracePoint's firmware rework. See `TODO.md` for the outstanding task list, and `BUGS.local.md` for the running bug log.
...
6. Never run `git commit` (or `git push`). Propose the commit message and let the user commit manually. Proposed commit messages and PR descriptions never include AI attribution trailers (`Co-Authored-By: Claude...`, `Claude-Session:`, or similar).
...
3. One task at a time, working through `TODO.md` in order.
...
The moment a fix is confirmed working, log it in `BUGS.local.md` before anything else, including the commit message. Every entry needs: Date, Ticket, Bug, Solution, Explanation (why it happened, why the fix works). Check for an existing entry before adding a new one — never log the same bug twice.
...
This project also uses a local, gitignored `CLAUDE.local.md` for additional maintainer-specific notes. It is not part of this repository's tracked history.
