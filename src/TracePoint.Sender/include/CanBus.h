...
class CanBus {
public:
    CanBus(int16_t tx_pin, int16_t rx_pin, TwaiSpeed speed, uint8_t tx_queueSize, uint8_t rx_queueSize)
    {
        ESP32Can.setPins(tx_pin, rx_pin);
        ESP32Can.setSpeed(speed);
        ESP32Can.setTxQueueSize(tx_queueSize);
        ESP32Can.setRxQueueSize(rx_queueSize);
